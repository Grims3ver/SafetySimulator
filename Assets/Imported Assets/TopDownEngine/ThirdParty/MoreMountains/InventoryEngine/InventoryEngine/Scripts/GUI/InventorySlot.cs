...
namespace MoreMountains.InventoryEngine
{
	...
	public class InventorySlot : Button 
	{
		...
		private GameObject cellphoneObj;
		private GameObject codexObj; 

		private Canvas cellphoneCanvas;
		...
		protected override void Start()
		{
			...
			//ADDED CODE HERE
			cellphoneObj = GameObject.Find("CellPhone_Canvas");
			codexObj = GameObject.Find("Codex_Canvas");

			if (cellphoneObj != null)
			{
				cellphoneCanvas = cellphoneObj.GetComponent<Canvas>();
				//print("got cell!");
			}

			if (codexObj != null)
			{
				codexCanvas = codexObj.GetComponent<Canvas>();
				//print("got codex!");
			}
		}
		...
		public virtual void SlotClicked () 
		{
			if (ParentInventoryDisplay!=null)
			{
				...
				//ADDED CODE HERE
				//THIS if statement toggles the cellphone and codex canvas' respectively
				if (Index == 0)
                {
					cellphoneCanvas.enabled = !cellphoneCanvas.enabled;
					if (codexCanvas.enabled)
					{
						codexCanvas.enabled = false;
					}
				} else if (Index == 1)
                {
					codexCanvas.enabled = !codexCanvas.enabled;
					if (cellphoneCanvas.enabled)
                    {
						cellphoneCanvas.enabled = false; 
                    }
