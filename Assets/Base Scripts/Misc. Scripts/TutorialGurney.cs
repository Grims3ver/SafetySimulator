...
public class TutorialGurney : MonoBehaviour
{
    ...
    //door specific 
    private ControlTutorialDoor[] doorCheck;
    ...
    void Start()
    {
        ...
        doorCheck = new ControlTutorialDoor[doors.Length];
        doorRenderer = new SpriteRenderer[doors.Length];

        for (int i = 0; i < doors.Length; ++i)
        {
            doorCheck[i] = doors[i].GetComponent<ControlTutorialDoor>();
    ...
    private void Update()
    {
        ...
        //unbarricade
        else if (playerNearby != -1 && doorCheck[playerNearby].doorIsBarricaded && doorBarricadeComplete && Input.GetKey(KeyCode.F))
        {
    ...
    private int GurneyNearbyDoor()
    {
        for (int i = 0; i < doors.Length; ++i)
        {
            ...
            if ((Mathf.Abs(doors[i].transform.position.x - this.transform.position.x) < 1.5) && (Mathf.Abs(doors[i].transform.position.y - this.transform.position.y) < 1.5))
            {
                return i;
            }
